...
import 'package:project_2/order_success_screen.dart';
...
class _OrderReviewScreenState extends State<OrderReviewScreen> {
  ...
  int _getFreeQuantity(
    ...
    final int buyQuantity =
        _toInt(offer["buyQuantity"]);

    final int freeQuantity =
        _toInt(offer["freeQuantity"]);
  ...
  double _getItemDiscount(
    Map<String, dynamic> item,
  ) {
    final subtotal =
        _getItemSubtotal(item);
  ...
  void _increaseQuantity(int index) {
    setState(() {
      final currentQuantity =
          _getQuantity(reviewItems[index]);

      reviewItems[index]["quantity"] =
          currentQuantity + 1;
    });
  }
  ...
  void _deleteItem(int index) {
    final deletedName =
        reviewItems[index]["name"]?.toString() ??
            "الصنف";
...
void _returnToCart() {
  ...
  // رقم مؤقت إلى حين ربط الباك
  final String orderNumber =
      "ORD-${sentAt.millisecondsSinceEpoch % 100000}";
  ...
  debugPrint("بيانات الطلبية:");
  debugPrint(orderData.toString());

  try {
    // مؤقتاً لمحاكاة إرسال الطلبية إلى الباك
...
OrdersStore.instance.addOrder(orderData);
    // تفريغ السلة فقط بعد نجاح الإرسال
    widget.onOrderSent?.call();

    // إزالة السلة والمراجعة من مسار الرجوع
    ...
    _showMessage(
      "حدث خطأ أثناء إرسال الطلبية",
    );

    debugPrint(
      "خطأ إرسال الطلبية: $error",
    );
  }

    // لاحقاً نفتح من هنا واجهة نجاح الإرسال.
  }
  ...
  Widget _buildPharmacyCard() {
    ...
    final String branch =
        widget.pharmacy["branch"]?.toString() ??
            "";
      ...
      child: Column(
        children: [
          ...
          _buildSummaryRow(
            ...
            value:
                "$totalPaidQuantity عبوة",
          ),
          _buildSummaryRow(
            title: "الكميات المجانية",
            value:
                "$totalFreeQuantity عبوة",
            valueColor:
                const Color(0xff169967),
          ),
